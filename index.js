...
//console.log('task3', artists[8].name);
...
function listOfNames(array) {
  ...
  for(let i=0; i<array.length; i++) {
    artistArr.push(array[i].name);
  }
  return artistArr;

}
console.log('task4',listOfNames(artists));
...
function removeArtist(array,number) {
  const removeByIndex = [...artists];

for(let i=0; i<array.length; i++) {
  removeByIndex.splice([number],1); 
...
function addArtist(array,name,years,genre,nationality,bio) {
  const newObj = [{name,years,genre,nationality,bio}];
  const addedArtists=[...artists, ...newObj];
 return addedArtists;
}
 
console.log('task6', addArtist(artists,'John Doe','1988-2022','Full Stack Development','African American','I have a background in customer service at Big Retail Chain. I am attending BloomTech to become a Frontend Developer.'));
